define(["dojo/_base/declare",
	...
	return declare(null, {
		...
		buildRepoObject: function(selectedRepo){
			...
			repoObject.accessToken = selectedRepo.authKey;
        ...
        getUrlRepoParts: function(url){
        	var searchTerm = "://";
        	var prefixIdx = url.indexOf("://") + searchTerm.length;
        	
        	var prefix = url.slice(0, prefixIdx);
        	url = url.slice(prefixIdx, url.length);
        	
        	searchTerm = "/";
        	var serverIdx = url.indexOf("/") + searchTerm.length;
        	var repo = encodeURIComponent(url.slice(serverIdx, url.length));
